...
export class HeaderComponent {
  ...
  constructor() { 
    setTimeout(() => this.header.set('Signalize !!'), 1000);
    setTimeout(() => this.header.update(header => header + '??' ), 5000)
    setInterval(() => this.taglineIndex.set(Math.floor(Math.random() * this._taglines.length)), 1000);
